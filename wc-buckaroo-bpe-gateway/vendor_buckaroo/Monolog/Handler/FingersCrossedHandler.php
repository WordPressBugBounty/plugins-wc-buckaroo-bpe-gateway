...
use BuckarooDeps\Monolog\Handler\FingersCrossed\ErrorLevelActivationStrategy;
use BuckarooDeps\Monolog\Handler\FingersCrossed\ActivationStrategyInterface;
use BuckarooDeps\Monolog\Logger;
use BuckarooDeps\Monolog\ResettableInterface;
use BuckarooDeps\Monolog\Formatter\FormatterInterface;
use BuckarooDeps\Psr\Log\LogLevel;
 ...
 * BuckarooDeps\Monolog\Handler\FingersCrossed\ namespace.
 *
 * @author Jordi Boggiano <j.boggiano@seld.be>
 *
 * @phpstan-import-type Record from \BuckarooDeps\Monolog\Logger
 * @phpstan-import-type Level from \BuckarooDeps\Monolog\Logger
 * @phpstan-import-type LevelName from \BuckarooDeps\Monolog\Logger
 */
class FingersCrossedHandler extends Handler implements ProcessableHandlerInterface, ResettableInterface, FormattableHandlerInterface
{
    ...
    public function __construct($handler, $activationStrategy = null, int $bufferSize = 0, bool $bubble = true, bool $stopBuffering = true, $passthruLevel = null)
    {
        ...
        if (!$this->handler instanceof HandlerInterface && !is_callable($this->handler)) {
            throw new \RuntimeException("The given handler (".json_encode($this->handler).") is not a callable nor a BuckarooDeps\Monolog\Handler\HandlerInterface object");
        }
    }
