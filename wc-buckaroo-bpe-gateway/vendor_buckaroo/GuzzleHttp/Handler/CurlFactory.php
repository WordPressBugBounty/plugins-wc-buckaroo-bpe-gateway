...
namespace BuckarooDeps\GuzzleHttp\Handler;

use BuckarooDeps\GuzzleHttp\Exception\ConnectException;
use BuckarooDeps\GuzzleHttp\Exception\RequestException;
use BuckarooDeps\GuzzleHttp\Promise as P;
use BuckarooDeps\GuzzleHttp\Promise\FulfilledPromise;
use BuckarooDeps\GuzzleHttp\Promise\PromiseInterface;
use BuckarooDeps\GuzzleHttp\Psr7\LazyOpenStream;
use BuckarooDeps\GuzzleHttp\TransferStats;
use BuckarooDeps\GuzzleHttp\Utils;
use BuckarooDeps\Psr\Http\Message\RequestInterface;
use BuckarooDeps\Psr\Http\Message\UriInterface;
...
class CurlFactory implements CurlFactoryInterface
{
    ...
    private static function createRejection(EasyHandle $easy, array $ctx): PromiseInterface
    {
        ...
        if ('' !== $sanitizedError) {
            $redactedUriString = \BuckarooDeps\GuzzleHttp\Psr7\Utils::redactUserInfo($uri)->__toString();
    ...
    private static function sanitizeCurlError(string $error, UriInterface $uri): string
    {
        ...
        $redactedUriString = \BuckarooDeps\GuzzleHttp\Psr7\Utils::redactUserInfo($baseUri)->__toString();
    ...
    private function applyHandlerOptions(EasyHandle $easy, array &$conf): void
    {
        ...
        if (!isset($options['sink'])) {
            ...
            $options['sink'] = \BuckarooDeps\GuzzleHttp\Psr7\Utils::tryFopen('php://temp', 'w+');
        }
        $sink = $options['sink'];
        if (!\is_string($sink)) {
            $sink = \BuckarooDeps\GuzzleHttp\Psr7\Utils::streamFor($sink);
